...
function loadDischargeType(){
  fetch("http://localhost/LaravelApiFile/loadDischargeType",{
    method: "POST",
    headers: {
  ...
  .then((result) => {
    ...
    $(result).each(function (k, v) {
      ...
      data +=
      "<option value='" + v.id + "'>" + v.discharge_type + "</option>";
              
    });
    $("#drpDischargeType").html("");
    $("#drpDischargeType").html(data);
  })
  .catch((error) => {
    console.log(error);
  });
...
function loadInstituteDropDown(){
  // ---------------------------------------------------------------------------
    ...
    fetch("/openmrs/ws/rest/v1/location", requestOptions)
            ...
            .then(function (result) { 
              ...
              $(result.results).each(function (k, v) {
                html+="<option value="+v.uuid+" >"+v.display+"</option>";
              });
              
              $('#drpInstitute').html(html);  
...
$('#btnSubmit').on('click', function(){
    ...
    var txtDischargeTime=$('#txtDischargeTime').val();
    var drpWardValue=2;
    ...
    var tranfer_institute_uuid=$('#drpInstitute :selected').val();
  ...
  }else if(tranfer_institute_uuid == ""){
    iziToast.error({position: "bottomRight", title: '', message: 'Tranfer Institute be empty!'});
  }
  else
  {
    fetch("http://localhost/LaravelApiFile/saveExternalWardTransfer", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
             ...
             txtDischargeTime: txtDischargeTime,
             ...
             person_id:person_id,
             tranfer_institute_uuid:tranfer_institute_uuid,
